...
# For HDB, PADS or PIVOTAL-HDB, repo name should not include version
if [[ $REPO_NAME == hdb-ambari-plugin* ]]; then
  REPO_NAME="HDB-AMBARI-PLUGIN"
  REPO_FILE_NAME="HDB-AMBARI-PLUGIN"
elif [[ $REPO_NAME == hdb* ]]; then
  REPO_NAME="HDB"
  REPO_FILE_NAME="HDB"
elif [[ $REPO_NAME == pivotal-hdb* ]]; then
  REPO_NAME="PIVOTAL-HDB"
  REPO_FILE_NAME="PIVOTAL-HDB"
elif [[ $REPO_NAME == PADS* ]]; then
  REPO_NAME="PADS"
  REPO_FILE_NAME="PADS"
fi
shopt -u nocasematch

#Add ProxyPass line if running in a sandbox VM
SANDBOX_HUE_CONF="/etc/httpd/conf.d/hue.conf"
if [[ $HOSTNAME == sandbox* && -f $SANDBOX_HUE_CONF ]]; then
   PROXY_PASS="\ \ ProxyPass /$REPO_NAME !"
   grep "$PROXY_PASS" $SANDBOX_HUE_CONF > /dev/null
   if [ $? -eq 1 ];then
     sed -i "4i  $PROXY_PASS" $SANDBOX_HUE_CONF
     service httpd restart > /dev/null
   fi
fi

# for HDB, PADS or PIVOTAL-HDB, remove the previous link, if it exists
if [ -e ${DOCUMENT_ROOT}/${REPO_NAME} ]; then
  rm ${DOCUMENT_ROOT}/${REPO_NAME}
  if [ $? -ne 0 ]; then
    error_exit "Unable to remove a link at ${DOCUMENT_ROOT}/${REPO_NAME}. Please check the ownership and directory permissions."
  fi
fi
